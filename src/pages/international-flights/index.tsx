...
function InternationalFlights() {
        ...
        fetch('https://api.nixtour.com/api/Web/AirlineList')
            ...
            .then(data => {
                if (data.Success && Array.isArray(data.Data)) {
                    const internationalAirlines = data.Data.filter((airline: Airline) => airline.AirlineType == 'International');
                    setAirlines(groupAirlinesByAlphabet(internationalAirlines));
                } else {
                    setAirlines({});
                }
                setLoading(false);
    ...
    // Filter out empty alphabet sections
    ...
    return (
        <div className='min-h-screen bg-white'>
            ...
            <div className="bg-[#2073C7] py-20 xs:py-24 sm:py-28 md:py-32 text-center">
                <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-2">International Airlines</h1>
            </div>
            {/* Flight Search UI */}
            ...
            {/* International Airline List */}
            <div className="max-w-[90%] md:max-w-6xl mx-auto px-2 xs:px-4 pb-16">
                <h2 className="text-xl xs:text-2xl font-bold mb-6 text-[#1cb0f6]">International Airline List</h2>
                {loading ? (
